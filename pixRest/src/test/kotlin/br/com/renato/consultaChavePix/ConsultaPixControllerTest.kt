...
internal class ConsultaPixControllerTest {
    ...
    @AfterEach
    fun teardown() {
        Mockito.reset(consultaStub)
    }

    @Test
    fun `deve consultar chave com dados validos`() {
    ...
    @Test
    fun `deve retornar NOT_FOUND quando chave pix nao for encontrada`() {
        val idPix = UUID.randomUUID()
        val idCliente = UUID.randomUUID()
    ...
    @Test
    fun `deve retornar BAD_REQUEST quando id cliente tiver formato invalido`() {
        val idPix = UUID.randomUUID()
        val idCliente = 123
